...
// Import dari library baru sesuai saran Anda
import { fetchPublicOrders, trackPublicOrder } from "../../lib/publicOrders";
import { Search, Filter, Ticket, Clock, Eye, X, CreditCard, Image as ImageIcon } from "lucide-react";
...
export default function Tracking() {
  ...
  // Fungsi load yang sudah disederhanakan sesuai saran Anda
  async function load() {
    try {
      setLoading(true);
      const rows = await fetchPublicOrders(); // Langsung menerima array
      setOrders(rows ?? []);
    } catch (e) {
  ...
  // Fungsi Lihat Detail yang sudah disederhanakan
  async function handleViewDetail(ticketCode) {
    try {
      setLoadingDetail(true);
      const detail = await trackPublicOrder(ticketCode); // Langsung menerima object
  ...
  // Logic Filter & Search tetap dipertahankan
  const filteredOrders = orders.filter((o) => {
    const matchesStatus = filterStatus === "ALL" || o.order_status?.toUpperCase() === filterStatus.toUpperCase();
    const matchesSearch = o.ticket_code?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const getStatusStyle = (status) => {
    ...
    if (s === "pending" || s === "process") return "text-orange-400 bg-orange-500/10 border-orange-400/20";
    ...
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white/5 p-6 rounded-[2.5rem] border border-white/10 backdrop-blur-md shadow-2xl">
        <div className="md:col-span-2 relative">
          ...
          <input 
            type="text"
            placeholder="Cari Tiket Anda (Misal: CA-X1ORDU)"
            value={searchQuery}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr><td colSpan="6" className="p-20 text-center text-white/20 animate-pulse">Loading data antrean...</td></tr>
              ) : filteredOrders.length === 0 ? (
                <tr><td colSpan="6" className="p-20 text-center text-white/20 italic font-bold tracking-widest uppercase">Pesanan tidak ditemukan.</td></tr>
              ) : (
...
{/* Modal Detail (Hanya Menampilkan Data & Foto sesuai Backend) */}
{selectedOrder && (
    ...
    <div className="relative w-full max-w-2xl bg-[#184832] border border-white/20 rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {/* Menampilkan Admin Note jika ada */}
